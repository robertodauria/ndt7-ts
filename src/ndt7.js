...
async function downloadTest(config, userCallbacks, urlPromise) {
  ...
  const workerfile = config.downloadworkerfile || new URL('./ndt7-download-worker.js', import.meta.url);
...
async function uploadTest(config, userCallbacks, urlPromise) {
  ...
  const workerfile = config.uploadworkerfile || new URL('ndt7-upload-worker.js', import.meta.url);
...
export { discoverServerURLs, downloadTest, uploadTest, test };